    ...
    try {
      ...
      if (type === 'customer'){
        ...
        const itemsList = arr.map(function(it){
          ...
          return `<li class=\"flex items-center justify-between\"><span>${qty}× ${nm}</span>${price?`<span class=\"text-slate-500\">${price}</span>`:''}</li>`;
        }).join('');
        html = `
          <div class=\"space-y-2\">
            ...
            ${arr.length ? `<ul class=\"space-y-1\">${itemsList}</ul>` : '<div class=\"text-slate-500\">—</div>'}
